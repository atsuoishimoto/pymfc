...
public:
	...
	PymRePtr():m_buf(NULL), m_pos(ULONG_MAX){}
	
	PymRePtr(PymBuffer &buf, long pos):m_buf(&buf), m_pos(pos) {
	
	}

	PymRePtr(const PymRePtr &r):m_buf(r.m_buf), m_pos(r.m_pos) {
	}

	bool isValid() {
		return m_buf && m_pos != ULONG_MAX && m_pos <= m_buf->size();
	}

	PymRePtr &operator =(const PymRePtr &r) {
		m_buf = r.m_buf;
		m_pos = r.m_pos;
		return *this;
	}
	...
	wchar_t operator *() {
		return m_buf->get(m_pos);
	}

	wchar_t operator[](long idx) {
		return m_buf->get(m_pos+idx);
	}
	PymRePtr operator +(long n) {
		return PymRePtr(*m_buf, m_pos+n);
	}
	PymRePtr &operator++(){
		m_pos++; return *this;
	}
	
	PymRePtr operator++(int){ 
		PymRePtr ret(*this); 
		m_pos++; 
		return ret;
	}
	...
	long operator -(const PymRePtr& r) {
	...
	bool operator ==(const PymRePtr &r) {
...
inline long operator -(int n, const PymRePtr &r) {
	return n - r.m_pos;
}



#include "pymeditor_sre.h"



/* name of this module, minus the leading underscore */
#if !defined(SRE_MODULE)
#define SRE_MODULE "pymfclib_bufsre"
#endif
...
LOCAL(int)
SRE_AT(SRE_STATE* state, PymRePtr ptr, SRE_CODE at)
...
//        return ((void*) ptr == state->end);
    ...
    case SRE_AT_BOUNDARY:
        if (state->beginning == state->end)
            return 0;
        that = (ptr > state->beginning) ?
//        that = ((void*) ptr > state->beginning) ?
            SRE_IS_WORD((int) ptr[-1]) : 0;
//        this_ = ((void*) ptr < state->end) ?
        this_ = (ptr < state->end) ?
...
{
    SRE_CODE chr;
    PymRePtr ptr = state->ptr;
    PymRePtr end = state->end;
        ...
        case SRE_OP_GROUPREF:
            ...
            i = ctx->pattern[0];
            {
                ...
                if (groupref >= state->lastmark) {
                    ...
                    int p = state->mark[groupref];
                    int e = state->mark[groupref+1];
...
//                    SRE_CHAR* e = (SRE_CHAR*) state->mark[groupref+1];
				...
				PymRePtr pp(state->beginning+p-1);
				PymRePtr ee(state->beginning+e-1);

				while (pp < ee) {
					if (ctx->ptr >= end || *ctx->ptr != *pp) {
                        RETURN_FAILURE;
					}
                    pp++; ctx->ptr++;
                }
...
//                    SRE_CHAR* e = (SRE_CHAR*) state->mark[groupref+1];
                    ...
                    while (pp < ee) {
                        if (ctx->ptr >= end ||
                            state->lower(*ctx->ptr) != state->lower(*pp))
                            RETURN_FAILURE;
                        pp++; ctx->ptr++;
                    }
                }
...
LOCAL(int)
SRE_LITERAL_TEMPLATE(PymRePtr ptr, int len)
...
getstring(PyObject* string, int* p_length, int* p_charsize)
{
	...
	PymBuffer *buf = (PymBuffer *)PyMFCPtr::asPtr(PyDTObject(string, false).callMethod("getBuffer", "").get());
	*p_length = buf->size();
	*p_charsize = 1;
	return PymRePtr(*buf, 0);
#if 0
...
inline 
int STATE_OFFSET(SRE_STATE *state, int member) {
	return  ((int)member - state->beginning.m_pos);
}

inline 
int STATE_OFFSET(SRE_STATE *state, PymRePtr &member) {
	return  (member.m_pos - state->beginning.m_pos);
}
...
state_getslice(SRE_STATE* state, int index, PyObject* string, int empty)
{
    ...
    if (string == Py_None || index >= state->lastmark || !state->mark[index] || !state->mark[index+1]) {
        ...
        i = STATE_OFFSET(state, state->mark[index]-1);	
        j = STATE_OFFSET(state, state->mark[index+1]-1);
...
//                match->mark[j+3] = ((char*) state->mark[j+1] - base) / n;
                match->mark[j+2] = (state->mark[j] - base - 1);
                match->mark[j+3] = (state->mark[j+1] - base - 1);
            } else
...
static PyObject*
pattern_subx(PatternObject* self, PyObject* template_, PyObject* string,
             int count, int subn)
{
    SRE_STATE state;
    PyObject* list;
...
pattern_sub(PatternObject* self, PyObject* args, PyObject* kw)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kw, "OO|i:sub", kwlist,
                                     &template_, &string, &count))
        return NULL;

    return pattern_subx(self, template_, string, count, 0);
}
...
pattern_subn(PatternObject* self, PyObject* args, PyObject* kw)
{
    ...
    return pattern_subx(self, template_, string, count, 1);
}
...
{
    PyObject* template_;
    if (!PyArg_ParseTuple(args, "O:expand", &template_))
        return NULL;

    /* delegate to Python code */
    return call(
        SRE_MODULE, "_expand",
        PyTuple_Pack(3, self->pattern, self, template_)
        );
}
